...
public class suanfa {
	...
	static int value[] = new int[100];//定义一个数组value，用来存储物品价值
	static int property[][] = new int[100][100];//定义一个数组property，用来存储物品的属性
	static int reproperty[] = new int[100];//定义一个数组reproperty，物品属性的当前值
	static int maxConstraints[] = new int[100];//定义一个数组maxConstraints，存储物品属性的最大约束条件
	static int bestvalue, res;//bestvalue为当前最大价值
	...
	static boolean check(int x, int revalue) {
		// 限界,减去不可能包含最优解的情况
		int res = 0;
		for (int i = x + 1; i <= n; i++) {
			res += value[i];
	...
	static void BackTrack(int x, int revalue) {
		if (x > n) {
